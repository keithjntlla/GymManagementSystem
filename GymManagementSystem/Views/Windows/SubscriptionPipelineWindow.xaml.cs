    ...
    public partial class SubscriptionPipelineWindow : Window
    {
        ...
        private void LoadPipeline()
        {
            ...
            try
            {
                ...
                FROM Payments 
                WHERE MemberID = @mid AND Date(NewExpiryDate) >= Date('now')
                ...
                try
                {
                    using (var conn = new SQLiteConnection(DatabaseHelper.ConnectionString))
                    {
                        ...
                        using (var trans = conn.BeginTransaction())
                        {
                            ...
                            // 2. Clear out the queued payment ledger record
                            string deleteSql = "DELETE FROM Payments WHERE PaymentID = @pid";
                            ...
                            string maxExpirySql = "SELECT MAX(Date(NewExpiryDate)) FROM Payments WHERE MemberID = @mid";
                            string newMaxExpiry = "-";
                            ...
                            string refundLogSql = @"
                                INSERT INTO Payments (MemberID, MemberName, AmountPaid, TotalAmount, Change, PaymentMode, MembershipType, DateOfTransaction, NewExpiryDate, DiscountAmount) 
                                VALUES (@mid, @mname, 0, @amt, 0, 'Refund', @type, @date, @expiry, 0)";
